...
func (gaish GetAllNVLinkInterfaceHandler) Handle(c echo.Context) error {
	...
	if len(instanceIDStrs) > 0 {
		...
		instances, _, err := computesvc.New(gaish.dbSession).GetInstances(ctx, nil, cdbm.InstanceFilterInput{InstanceIDs: instanceIDs}, cdbp.PageInput{Limit: cdb.GetIntPtr(cdbp.TotalLimit)})
		if err != nil {
